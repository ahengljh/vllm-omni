...
@pytest.mark.parametrize("omni_runner", pd_test_params, indirect=True)
...
@hardware_test(res={"cuda": "H100"}, num_cards=3)
@pytest.mark.parametrize("omni_runner", pd_test_params, indirect=True)
def test_pd_text_to_audio(omni_runner, omni_runner_handler) -> None:
    """Test PD disaggregation with text input and audio output
    through the full 4-stage pipeline (prefill -> decode -> talker -> code2wav)."""
    request_config = {
        "prompts": get_question("text"),
        "modalities": ["audio"],
    }
